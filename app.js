...
>>>>>>> d646b21905abb1077c6c70f60c98ed2f70c6c0b9
        ...
        window.app.exchange.methods.exchangeForHOP(cost).send({ from: address }).then(async () => {
            ...
            $("#exchange_progress").html("交易成功")
            syncBalance()
        }).catch(e => {
            // console.log(e.message);
            $("#exchange_progress").html("交易失败"+e.message)
        })
    })

    $("#claim_btn").click(async () => {
        // if ($("#claimable").html() > 0) {
        $("#claim_progress").html("交易进行中...")
        let n = $("#claimable").html()
        window.app.exchange.methods.claimHOP(n).send({ from: window.app.current_account }).then(async () => {
            alert("claim succeed!")
            $("#claim_progress").html("交易成功")
            syncBalance()
        }).catch(e => {
            $("#claim_progress").html("交易失败"+e.message)
        })
